...
if __name__ == "__main__":
    ...
    for filename in filenames:
        ...
        img = cv2.imread(deskewed_file, 0)
        mask = cv2.imread(filename[:-4]+"_sep.png", 0)
        kernel = np.ones((3,3),np.uint8)
        mask = cv2.dilate(mask,kernel,iterations = 1)

        bin_img = ~(img > threshold_otsu(img))*1
        gray_img = ((1-bin_img)*255).astype('uint8')
        # cv2.imwrite(filename[:-4]+"_gray.png", gray_img)
        # cv2.imwrite(filename[:-4]+"_mask.png", 255-mask)

        cleaned_img = gray_img + mask
        cv2.imwrite(filename[:-4]+"_cleaned.png", cleaned_img)

        # output_dir = os.path.dirname(filename)
        # linecut = detect_lines(filename[:-4]+"_cleaned.png", output_dir, epsilon=2, type='not')
        linecuts, debug_im  = detect_lines(filename[:-4]+"_cleaned.png")
        cv2.imwrite(filename[:-4] + "_out.png", debug_im)
        # for box in table_boxes:
        #     l, t, r, b = box
        #     cv2.rectangle(im, (l, t),
        #                   (r, b), (0, 0, 255), 2)
        # cv2.imwrite(filename[:-4]+"_rec.png", im)

        cv2.imwrite(filename[:-4] + "_out.png", im)
        # get_linecut_pos(filename[:-4]+"_cleaned.png", filename[:-4]+"_out.png", 
        #     os.path.dirname(filename), filename[:-4]+"_sep.png", table_boxes)

        # for i in linecut:
        #     cleaned_img = cv2.drawContours(cleaned_img, [i], 0, (0, 0, 255), 2)
        # cv2.imwrite(filename[:-4]+"_debug.png", cleaned_img)

        # get_linecut_pos(filename[:-4]+"_cleaned.png", "sjnk_results/", filename[:-4]+"_sep.png", table_boxes)

        # os.remove(filename[:-4]+"_sep.png")
        # os.remove(filename[:-4]+"_out.png")
        # os.remove(filename[:-4]+"_cleaned.png")
        # os.remove(filename[:-4]+"_bin.png")
