...
def area(a):
    ...
    return max(a[0].stop-a[0].start,0) * max(a[1].stop-a[1].start,0)
...
# calculate distance from tuple x = (u,v)
def dist(x ,y):
    u = x[0] - y[0]
    v = x[1] - y[1]
    return (u**2 + v**2)**0.5
...
def pad_box(box, pad, max_shape):
    left, top, right, bottom = box
    h, w = max_shape

    left = left - pad if left >= pad else 0
    top = top - pad if top >= pad else 0
    right = right + pad if right + pad < w else w - 1
    bottom = bottom + pad if bottom + pad < h else h - 1
